...
const Search = ({ itemsFile }: { itemsFile: string }) => {
  ...
  const { uniqueKeys, loadNames } = useDataHandling(
    loadItems,
    config,
    setConfig,
    setNames
  ...
  useEffect(() => {
    ...
    if (searchBarRef.current && searchBarInitialPosition.current === null) {
      searchBarInitialPosition.current =
    ...
    <main className={`px-4`}>
      <div id="search">
        <div className="mb-6 pt-8">
          Fetching from{" "}
          <span className="inline p-2 rounded-md">{itemsFile}</span>
        </div>
        {error && <div className="error-message">Error: {error.message}</div>}
        <div
          className="flex justify-stretch h-[120px]"
          id="search-bar"
          ref={searchBarRef}
        >
          <select
